   ...
   def decode(ns: List[Int]): Either[Exception, List[Int]] =
      def f(ns: List[Int]): List[Int] =
         Integer.parseUnsignedInt(
           ns.map(_.toBinaryString.grouped(8).mkString)
              .map(s => "0" * (8 - s.length) + s)
              .map(_.drop(1))
              .mkString,
           2) :: Nil
...
private def splitAtUnder128(list: List[Int], current: List[Int] = Nil, acc: List[List[Int]] = Nil): List[List[Int]] =
   list match
      case Nil                        => if current.nonEmpty then (current :: acc).reverse else acc.reverse
      case head :: tail if head < 128 => splitAtUnder128(tail, Nil, (head :: current).reverse :: acc)
      case head :: tail               => splitAtUnder128(tail, head :: current, acc)
